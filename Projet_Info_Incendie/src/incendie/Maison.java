...
public class Maison extends Case implements Propagation {
	 ...
	 * @return un objet Maison avec une intensité, un etat et une présence de pompier donnés.
	 */
	public Maison(int colonne_j, int ligne_i, int intensite, EtatCase etat_case, Boolean presence_pompier){
		super(colonne_j, ligne_i, intensite, etat_case, presence_pompier);
		this.colonne_j = colonne_j;
		this.ligne_i = ligne_i;
		this.nature_case = NatureCase.MAISON;
	...
	/**
	 * Cette méthode augmente l'intensité des cases de type MAISON de 2. Si l'intensité atteind 5, l'état de la case devient CARBONISE.
	 */
	public void  gestion_intensite(){
		if (this.etat_case == EtatCase.FEU){
			this.intensite = this.intensite + 2;
			if (this.intensite > 5){
	...
	public String toString(){
		if (this.etat_case != EtatCase.CARBONISE){
			return "La Maison se situant à la position " + "[" + this.ligne_i + "," + this.colonne_j + "]" + " a pour intensité " + this.intensite + "." + " L'état de la case est " + this.etat_case + ".";
		}
		else{
			return "La Maison se situant à la position " + "[" + this.ligne_i + "," + this.colonne_j + "]." + " L'état de la case est " + this.etat_case + " donc son intensité est de " + this.intensite + ".";
		}
